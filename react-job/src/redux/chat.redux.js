...
export const chat = (state = initState, action) => {
  switch (action.type) {
    ...
    case MSG_READ:
      ...
      return {
        ...state,
        chatmsg: state.chatmsg.map(v => ({ ...v, read: v.from === from })),
...
// action
// -------------------------------------------
export const msgLisg = (msgs, users, userid) => ({
  type: MSG_LIST,
  payload: { msgs, users, userid }
});
export const msgRecv = (msg, userid) => ({
  type: MSG_RECV,
  payload: { msg, userid }
});
export const readMsg = (from, readNum) => ({
  type: MSG_READ,
  payload: { from, readNum }
});

// Redux Async Example: https://github.com/reduxjs/redux/tree/master/examples/async
// effects:
// ----------------------------------------
// 从数据库中获取 msg 列表
